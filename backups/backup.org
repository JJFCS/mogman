...
(use-package temple-os-theme        :vc (:url "https://github.com/Senka07/temple-os-emacs-theme"))
...
;; NOTE - some themes do not define faces such as 'shadow', 'warning', 'error', etc.. then this fails..
;; NOTE - at that point, do not bother using the theme

(advice-add 'load-theme :after (lambda (&rest _)
    (let* (
            (something)
            (bg      (face-attribute 'default :background nil t))
            (subtle  (color-blend (color-name-to-rgb (face-attribute 'shadow :foreground nil t)) (color-name-to-rgb bg) 0.5))  ;; NOTE - 0.0 == pure shadow, 1.0 == pure bg
            (warning (face-attribute 'warning :foreground nil t))
            (error   (face-attribute 'error   :foreground nil t))

            )

        ;; TODO - can experiment with which whitespace setting to move to what category

        (dolist (face '(whitespace-space whitespace-tab whitespace-hspace whitespace-newline whitespace-line))
            (set-face-attribute face nil :foreground (apply #'color-rgb-to-hex subtle) :background 'unspecified))
        (dolist (face '(whitespace-empty whitespace-space-after-tab whitespace-indentation))
            (set-face-attribute face nil :foreground warning :background 'unspecified))
        (dolist (face '(whitespace-trailing whitespace-space-before-tab))
            (set-face-attribute face nil :foreground error :background error))
    )
                                   )
    )
...
(use-package orderless
    ...
    (setq orderless-matching-styles '(orderless-initialism orderless-prefixes orderless-flex orderless-regexp))
        )
#+end_src
