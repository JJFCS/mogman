...
$ function 4
#+begin_src emacs-lisp
(defun onncera-insert-double-dagger () (interactive) (insert "‡"))
(defun onncera-insert-plus-minus-sign () (interactive) (insert "±"))
(defun onncera-insert-currency-sign () (interactive) (insert "¤"))
#+end_src
...
(set-face-attribute 'completions-group-title nil :slant 'normal)
...
(use-package minibuffer
    ...
    :hook ((minibuffer-setup . onncera-minibuffer-truncate-lines))
    ...
    (setq completions-sort 'alphabetical)  ;; alternative - 'historical'
...
(use-package consult :ensure t
    :config
    (setq consult-buffer-filter '("\\` " "\\`\\*")))  ;; hides star (*) buffers - press 'SPC' to see
#+end_src

$ ORG
#+begin_src emacs-lisp
(setq org-startup-with-inline-images t)
(setq org-fontify-quote-and-verse-blocks t)  ;; force org to use org-quote face to text inside quote blocks

(font-lock-add-keywords 'org-mode '(
    ("¤\\([^¤]+\\)¤" (1 '(:background "blue"          :foreground "white") prepend))
    ("‡\\([^‡]+\\)‡" (1 '(:background "deep pink"     :foreground "white") prepend))
    ("±\\([^±]+\\)±" (1 '(:background "DarkGoldenrod" :foreground "white") prepend))
        )
    )
#+end_src
...
(defvar onncera-u-map (make-sparse-keymap) "onncera C-c u prefix")     ;; unicode
(global-set-key (kbd "C-c u") onncera-u-map)
(define-key onncera-u-map (kbd "b") #'onncera-insert-currency-sign)    ;; blue highlights
(define-key onncera-u-map (kbd "p") #'onncera-insert-double-dagger)    ;; pink highlights
(define-key onncera-u-map (kbd "y") #'onncera-insert-plus-minus-sign)  ;; yellow highlights
